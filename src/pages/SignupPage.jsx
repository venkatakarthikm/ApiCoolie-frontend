...
export function SignupPage() {
  ...
  const [timezoneDefault, setTimezoneDefault] = useState('UTC');
  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center px-4 py-12 relative overflow-hidden bg-background">
      <Helmet>
        <title>Create Account | Api Coolie</title>
      </Helmet>
      ...
      <div className="max-w-md w-full space-y-6 relative z-10">
        ...
        <div className="border border-border/40 bg-card/60 backdrop-blur-md rounded-3xl p-8 shadow-xl space-y-5">
          {error && (
          ...
          <form onSubmit={handleSignup} className="space-y-3.5">
            <div className="space-y-1.5">
              <label className="block text-[10px] font-extrabold uppercase text-muted-foreground tracking-wider" htmlFor="fullName">Full Name</label>
            ...
            <div className="space-y-1.5">
              <label className="block text-[10px] font-extrabold uppercase text-muted-foreground tracking-wider" htmlFor="timezone">Default Timezone</label>
              <select
                id="timezone"
                value={timezoneDefault}
                onChange={(e) => setTimezoneDefault(e.target.value)}
                className="w-full px-3 py-2.5 border border-border rounded-xl text-sm bg-background/50 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/20 transition-all"
              >
                <option value="UTC">Coordinated Universal Time (UTC)</option>
                <option value="Asia/Kolkata">India Standard Time (IST)</option>
                <option value="America/New_York">Eastern Time (EST/EDT)</option>
                <option value="Europe/London">London / Greenwich Time (GMT)</option>
              </select>
            </div>
